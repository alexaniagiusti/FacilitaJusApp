	...
	export default {
		...
		methods: {
			jaAdicionado(item) {
				...
				this.itemsSelecionados.map(i => i.id === item.id ? add = true : false)

				return !add
			},
			querySelections (v) {
				this.itemsFiltrados = this.items.filter(e => {
					return e.city.match(this.criarPadrao(v))
				})
			},
			criarPadrao(str) {
				var accented = {
			...
			addItem(item) {
				var repetido = false
				this.itemsSelecionados.map(i => i.id === item.id ? repetido = true : null)
				
				if(repetido) {
					//this.$store.dispatch('snackbar_info', 'Você já selecionou ' + item.city )
				} else {
